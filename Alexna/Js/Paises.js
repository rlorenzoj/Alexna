const Paises = (function () {
    ...
    function Inicio() {
        ...
        (function () {
            ...
            $('body').on('click', 'i.editRow', function (e) {
                ...
                modalPaises.EstablecerAceptar(callback);
                modalPaises.Mostrar(id);
            });
            ...
            $('body').on('click', 'i.deleteRow', function (e) {
                const id = $(this).data('id');
                const ok = function () {
                    const url = '/Services/SvcPaises.svc/BorrarPais';
                    const type = 'POST';
                    const params = {
                        id: id
                    };
                    ...
                    const ok = function (datos, textStatus, jqXHR) {
                        if (datos) {
                            ...
                            Dolf.Ventana.WarningModal('Países', 'No se ha podido borrar el país indicado.');
                        }
                    }
    ...
    function pLoadPaises() {
        const dataTable = $('#tablaPrincipal').DataTable({
            ajax: {
                url: '/Services/SvcPaises.svc/ObtenerListado',
                method: 'POST',
            ...
            columns: [
                ...
                { data: 'Pais_Observaciones' },
                {
                    ...
                    render: function (data, type, row, meta) {
                        return '<span style="font-size: 20px;"><i class="fas fa-pencil-alt editRow mr-3 pointer" data-id="{0}"></i><i class="fas fa-trash-alt deleteRow pointer" data-id="{0}"></i></span>'.formato(row.Pais_Id);
                    }
                }
            ],
            paging: true,
